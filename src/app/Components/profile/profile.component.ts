  ...
  private loadUserData(): void {
    ...
    if (userId) {
      this.userService.getUSerById(userId).pipe(
        ...
        this.surname_2.setValue(userData.surname_2);
        this.alias.setValue(userData.alias);
        this.birth_date.setValue(formatDate(userData.birth_date, 'yyyy-MM-dd', 'en'));
        this.email.setValue(userData.email);
  ...
  updateUser(): void {
    ...
    if (userId) {
      this.userService.updateUser(userId, this.profileUser).pipe(
        ...
        const successResponse: ResponseError = {
          statusCode: 200,
          message: 'Update successful',
          messageDetail: 'Profile update was successful',
          code: 'UPDATE_SUCCESS',
          timestamp: new Date().toISOString(),
          path: '',
          method: ''
        };
        this.sharedService.managementToast('profileFeedback', true, successResponse);
      });
    }
