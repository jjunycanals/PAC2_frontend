...
export class PostFormComponent implements OnInit {
  ...
  private loadCategories(): void {
    this.categoriesList$ = of(this.localStorageService.get('user_id')).pipe(
      switchMap(userId => {
  ...
  private editPost(): Observable<boolean> {
    // let errorResponse: any;
    ...
    return this.postService.updatePost(this.postId!, this.post).pipe(
      map((updatedPost: PostDTO) => {
        return true;
      })
    );
  }

  private createPost(): Observable<boolean> {
    // let errorResponse: any;
    // let responseOK: boolean = false;
    // const userId = this.localStorageService.get('user_id');
    // if (userId) {
    ...
    return this.postService.createPost(this.post).pipe(
      map((createdPost: PostDTO) => {
        return true;
      })
    );;
  }
